module.exports = {
  saveJobs: async (req, res) => {
    ...
    const { date, job_status, job_name, job_company, job_link, job_notes } = req.body
    const [existingDate] = await db.check_date([date])

    if (existingDate) {
      await db.post_jobs([id, existingDate.id, job_status, job_name, job_company, job_link, job_notes])
    }

    if (!existingDate) {
      const [newDate] = await db.create_date([date])
      await db.post_jobs([id, newDate.id, job_status, job_name, job_company, job_link, job_notes])
    }
